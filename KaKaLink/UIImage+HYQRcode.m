...
//  TourGuidShop
//
//  Created by tenpastnine-ios-dev on 17/1/3.
...
+ (void)qrImageWithString:(NSString *)string size:(CGFloat)size completeion:(void (^)(UIImage *))completion
{
...
+ (void)qrImageWithString:(NSString *)string  size:(CGFloat)size iconImage:(UIImage *)iconImage scale:(CGFloat)scale completion:(void (^)(UIImage * image))completion {
    ...
    [UIImage qrImageWithString:string size:size CenterImageType:CenterImageType_Square iconImage:iconImage scale:scale completion:completion];
}

#pragma mark - 带图片二维码(图片指定比例、centerimagetype)
+ (void)qrImageWithString:(NSString *)string size:(CGFloat)size CenterImageType:(CenterImageType)type iconImage:(UIImage *)iconImage scale:(CGFloat)scale completion:(void (^)(UIImage *image))completion
{
...
+ (void)qrIamge:(UIImage *)qrImage addBgImage:(UIImage *)bgImage bgImageSize:(CGFloat) bgImageSize completion:(void (^)(UIImage *image))completion{
    [UIImage qrIamge:qrImage qrImageType:CenterImageType_Square addBgImage:bgImage bgImageSize:bgImageSize completion:completion];
}
#pragma mark - 给二维码设置背景
+ (void)qrIamge:(UIImage *)qrImage qrImageType:(CenterImageType)type addBgImage:(UIImage *)bgImage bgImageSize:(CGFloat) bgImageSize completion:(void (^)(UIImage *image))completion{
...
+ (UIImage *)qrcodeImage:(UIImage *)qrImage addIconImage:(UIImage *)iconImage centerImageType:(CenterImageType) type scale:(CGFloat)scale {

    // 图片放大倍数等于屏幕分辨率
    CGFloat screenScale = [UIScreen mainScreen].scale; // 屏幕分辨率
    ...
    if (iconImage)
    {
        CGSize acatarSize = CGSizeMake(rect.size.width*scale, rect.size.height*scale);
        CGFloat x = (rect.size.width-acatarSize.width)*0.5;
        CGFloat y = (rect.size.height-acatarSize.height)/2;
        if (type == CenterImageType_Circle)
        {
            iconImage = [UIImage createCircularImage:iconImage];
        }
        else if (type == CenterImageType_CornerRadious)
        {
            iconImage = [UIImage imageWithRoundedCorners:iconImage Size:acatarSize andCornerRadius:5.0f];
        }
        [iconImage drawInRect:CGRectMake(x, y, acatarSize.width, acatarSize.height)];
    }
...
{
    // 1.创建一个bitmap类型图片上下文（空白的UIImage）
    // NO 将来创建的透明的UIImage
    // YES 不透明
