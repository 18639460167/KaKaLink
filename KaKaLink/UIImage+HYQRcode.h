...
typedef NS_ENUM(NSInteger, CenterImageType) {
    // 方形
    CenterImageType_Square,
    // 圆形
    CenterImageType_Circle,
    // 切圆角
    CenterImageType_CornerRadious
};
 ...
 *  @param string     二维码文本
 *  @param size       二维码大小
 *  @param completion 二维码制作成功回调
 */
+ (void)qrImageWithString:(NSString *)string size:(CGFloat)size completeion:(void(^)(UIImage *image))completion;
