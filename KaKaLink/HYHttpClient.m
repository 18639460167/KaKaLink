...
{
    [self doService:action param:paramDic timeInterVal:timeInterValInt method:@"GET" isCard:NO callBack:callbackBlock];
}
...
{
    [self doService:action param:paramDic timeInterVal:timeInterValInt method:@"POST" isCard:NO callBack:callbackBlock];
}
...
#pragma mark - 卡券请求接口
+ (void)doCardGet:(NSString *)action param:(NSDictionary *)paramDic timeInterVal:(NSTimeInterval)timeInterValInt callback:(HYHttpCallback)callbackBlock
{
    [self doService:action param:paramDic timeInterVal:timeInterValInt method:@"GET" isCard:YES callBack:callbackBlock];
}

+ (void)doCardPost:(NSString *)action param:(NSDictionary *)paramDic timeInterVal:(NSTimeInterval)timeInterValInt callback:(HYHttpCallback)callbackBlock
{
    [self doService:action param:paramDic timeInterVal:timeInterValInt method:@"POST" isCard:YES callBack:callbackBlock];
}


#pragma mark - 请求处理
+ (void)doService:(NSString *)action param:(NSDictionary *)paramDic timeInterVal:(NSTimeInterval)timeInterValInt method:(NSString *)httpMethod isCard:(BOOL)isCard callBack:(HYHttpCallback)callBackBlock
{
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@",ServiceRoot,action]];
    if (isCard)
    {
        url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@",ServiceRoot1,action]];
    }
    NSString *postStr = @"";
    if (paramDic != nil)
    {
    ...
    NSURLSessionDataTask *dataTask = [[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        else
        {
            ...
            if ([jsonResult isKindOfClass:[NSDictionary class]])
            {
                ...
                {
                    hyResponse.errcode = [NSString trimNSNullAsNoValue:[jsonResult objectForKey:@"errcode"]];
