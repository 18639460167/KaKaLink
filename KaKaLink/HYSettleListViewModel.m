...
#import "HYSettleListViewModel.h"

@implementation HYSettleListViewModel
...
- (instancetype)init
{
    if (self = [super init])
    {
        ...
        self.pageIndex = 0;
        requestFail = NO;
        self.settleArray = [NSMutableArray new];
...
{
    if ((self.pageIndex + 1)*PAGESIZE >= self.totalCount)
    {
                ...
                if (response.mStatus == HY_HTTP_OK)
                {
                    ...
                    if ([result isKindOfClass:[NSDictionary class]])
                    {
                        ...
                        if ([list isKindOfClass:[NSArray class]])
                        {
                            ...
                            {
                                HYSettleListModel *model = [HYSettleListModel createWithDic:dic];
                                [settleArray addObject:model];
                            }
                        }
                        requestFail = NO;
